...
func fakeReply(t *testing.T, root, dir string, inputs []string, globs []globDependent) {
	...
	if err := os.WriteFile(filepath.Join(replyDir, "codemodel-v2-test.json"), []byte(`{"configurations":[]}`), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(replyDir, "cache-v2-test.json"), []byte(`{"entries":[]}`), 0o644); err != nil {
		t.Fatal(err)
	}
	index := `{"reply": {"codemodel-v2": {"jsonFile": "codemodel-v2-test.json"}, "cache-v2": {"jsonFile": "cache-v2-test.json"}, "cmakeFiles-v1": {"jsonFile": "cmakeFiles-v1-test.json"}, "toolchains-v1": {"jsonFile": "toolchains-v1-test.json"}}}`
...
func TestReconfigureReason(t *testing.T) {
	...
	// IDE consumers require the complete project model even when cmk's own
	// configure inputs are otherwise current.
	for _, test := range []struct {
		query string
		kind  string
	}{
		{"codemodel-v2", "codemodel"},
		{"cache-v2", "cache"},
		{"toolchains-v1", "toolchains"},
	} {
		if err := os.Remove(filepath.Join(dir, ".cmake/api/v1/reply/"+test.query+"-test.json")); err != nil {
			t.Fatal(err)
		}
		want := "cmake file API " + test.kind + " reply unavailable"
		if got := p.treeAt(dir, "").reconfigureReason(tc); got != want {
			t.Fatalf("missing %s reply: got %q, want %q", test.kind, got, want)
		}
		fakeReply(t, root, dir, []string{"CMakeLists.txt"}, nil)
	}
